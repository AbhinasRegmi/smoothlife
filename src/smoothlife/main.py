...
# base_grid: np.ndarray = random_init_center(np.zeros((GRID.HEIGHT, GRID.WIDTH)), 45)
...
def update(frame):
    ...
    update_grid = update_update_grid(base_grid)
    base_grid = update_base_grid(base_grid, update_grid)
    plot.set_array(base_grid)
    return plot,

animation = animation.FuncAnimation(fig, update, frames=range(10000), interval=1)
plt.show()